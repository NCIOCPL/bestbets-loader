...
class CategoryToMatchTransformer extends AbstractRecordTransformer {
     ...
     * @param {object} config.concurrencyLimit The maximum number of concurrent calls to the analyze endpoint.
     */
    constructor(logger, esclient, { tokenizer = false, filter = false, concurrencyLimit = 14} = {}) {
        super(logger);
        ...
        if(typeof concurrencyLimit !== 'number' || concurrencyLimit < 1)
          throw new Error('concurrencyLimit must be an integer greater than zero.')
        this.analyzeLimiter = pLimit(concurrencyLimit);

        // This is slightly hacky. Before making any other calls, the Elasticsearch
        // client first performs a "ProductCheck" to make sure it's running against
        // a supported version of the server. As with the analyzer call, these
        // requests can "stack up" with each one setting another listener and waiting
        // for the prior calls to return. This results in Node displaying a warning
        // of a possible memory leak. In this case, the message is a false alarm and
        // may be prevented by raising the limit to the maximum number of calls we
        // should be making. (Ideally we'd set that for the specific emitter instance
        // instead of the class, but that's not possible.)
        if(concurrencyLimit > 10 && concurrencyLimit > EventEmitter.defaultMaxListeners){
          EventEmitter.setMaxListeners(concurrencyLimit);
        }
    }
    ...
    async tokenizeNameWrap(name) {
        if (this.tokenCache[name.toLowerCase()]) {
            ...
            if (this.fetchQueue[name.toLowerCase()] ) {
                ...
                this.tokenCache[name.toLowerCase()] = await this.analyzeLimiter( () => this.tokenizeName(name) );
    ...
    async tokenizeName(name) {
        const ALLOWED_TOKEN_TYPES = ['<ALPHANUM>', '<NUM>'];
        ...
        return res.tokens.reduce(
          (ac, curr) => (ALLOWED_TOKEN_TYPES.includes(curr.type) ? (ac + 1) : ac ),
          0);
    }
     ...
     * @param {string} config.settingsPath The path to the BestBets index settings object
     * @param {string} config.analyzer The analyzer to use for tokenization from the settings
     * @param {string} config.concurrencyLimit The maximum allowed number of concurrent analyzer calls.
     */
    static async GetInstance(logger, config) {
        ...
        if(
            ( config['concurrencyLimit'] && typeof config.concurrencyLimit !== 'number')
            || config.concurrencyLimit < 1
          ) {
          throw new Error('concurrencyLimit must be an integer greater than zero.')
        }
        const concurrencyLimit = config.concurrencyLimit; // If not set, uses the constructor's default.
