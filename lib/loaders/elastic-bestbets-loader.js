...
const CATEGORY_RECORD_TYPE = 'categoryDisplay';
const SYNONYMS_RECORD_TYPE = 'synonyms';
    ...
    async loadRecord(matches) {
        ...
        const categoryDisplay = matches
          ...
          .map(cat => ({
            ...
            record_type: CATEGORY_RECORD_TYPE,
            name: cat.category,
            weight: cat.weight,
        ...
        const docArr = matches.reduce((ac, curr, ci) => [
          ...
          [ `${curr.contentID}_${ci}`, {
              ...
              record_type: SYNONYMS_RECORD_TYPE,
        ...
        try {
          res = await Promise.all([
            this.estools.indexDocumentBulk(this.indexName, docArr),
            this.estools.indexDocument(this.indexName, categoryDisplay.contentid, categoryDisplay)
          ]);
        } catch (err) {
        ...
        if (res[0].updated.length) {
            ...
            const message = `Category ${matches[0].contentID} had document errors`
            this.logger.error(message);
            throw new Error(message);
        }
    }
